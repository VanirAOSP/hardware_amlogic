...
int ath6kl_load_driver()
{
    ...
    snprintf(mod_path, SYSFS_PATH_MAX, "%s/%s.ko",WIFI_DRIVER_MODULE_DIR,ATH6K_DRIVER_KO_1);
    if(wifi_insmod(mod_path, DRIVER_MODULE_ARG) != 0){
    	wpa_printf(MSG_DEBUG, "%s: failed to insmod ATH6K_DRIVER_KO_1 \n", __func__);
    	return -1;
    }
	snprintf(mod_path, SYSFS_PATH_MAX, "%s/%s.ko",WIFI_DRIVER_MODULE_DIR,ATH6K_DRIVER_KO_2);
    if(wifi_insmod(mod_path, DRIVER_MODULE_ARG) !=0){
    	wpa_printf(MSG_DEBUG, "%s: failed to insmod ATH6K_DRIVER_KO_2 \n", __func__);
    	return -1;
    }
    
	snprintf(mod_path, SYSFS_PATH_MAX, "%s/%s.ko",WIFI_DRIVER_MODULE_DIR,ATH6K_DRIVER_KO_3);
    if(wifi_insmod(mod_path, DRIVER_MODULE_ARG) != 0){
